...
public class WxApi2Impl extends AbstractWxApi2 {
    ...
    public WxResp qrcode_create(Object scene_id, int expire_seconds) {
    	...
    	params.put("action_info", new NutMap().setv("scene", new NutMap().setv("scene_id", scene_id)));
    	return postJson("/qrcode/create", params);
    }
    
    public String qrcode_show(String ticket) {
    	try {
			return "https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=" + URLEncoder.encode("", Encoding.UTF8);
    ...
    @Override
    public WxResp template_send(String touser, String template_id, String topcolor, Map<String, WxTemplateData> data) {
    	return postJson("/template/send", "touser", touser, "template_id", template_id,
    					"topcolor", topcolor, 
    					"data", data);
    }
